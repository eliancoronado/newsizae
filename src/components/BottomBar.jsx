...
import { FaDiagramProject, FaUsers } from "react-icons/fa6";
...
const BottomBar = ({ activeTab, setActiveTab, totalUnread = 0, user }) => {
  ...
  const navItems = [
    ...
    { id: "community", icon: FaUsers, label: "Comunidad" },
  ];

  return (
    <>
      {/* Bottom Bar para móvil */}
      <div className="fixed bottom-0 h-[7vh] left-0 right-0 bg-[#201f1f]/90 backdrop-blur-xl border-t border-gray-800 z-50 md:hidden">
        <div className="flex items-center justify-around px-2 py-2">
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                ...
                {/* Bottom Bar para móvil <span className="text-[12px] font-medium">{item.label}</span>*/}
              </button>
            );
          })}
          <div
          ...
          >
            ...
            <img
              src={user?.picture}
              className="rounded-full h-9 w-9 md:h-2/3 md:w-auto object-cover"
              alt=""
            />
          </div>
        </div>
