...
const LoginApp = () => {
  ...
  async function loginWithGoogle() {
    ...
    try {
      ...
      // 2. Generar Custom Token usando tu función existente
      ...
      // 4. Guardar en localStorage (para la web)
      ...
      localStorage.setItem("apiUid", firebaseUser.uid);

      // 5. Crear el objeto con token y user data para enviar a la app
      const tokenData = {
      ...
      // 6. Codificar y abrir la app con deeplink
      const encodedData = encodeURIComponent(JSON.stringify(tokenData));

      // Para Android
      const androidDeepLink = `babooapp://auth?data=${encodedData}`;
      // Para iOS
      const iosDeepLink = `babooapp://auth?data=${encodedData}`;

      // Detectar plataforma y usar el deeplink correspondiente
      ...
      if (isAndroid) {
        ...
        // Si es web, mostrar mensaje o redirigir al dashboard web
        navigate("/dashboard");
      }
    } catch (error) {
